      FUNCTION rtsafeOLD(funcd, x1, x2, xacc)
...
*     Newton-Raphson, but with bisection
      ...
      DOUBLE PRECISION rtsafeOLD, x1, x2, xacc, temp
      DOUBLE PRECISION df, dx, dxold, f, fh, fl, xh, xl
      INTEGER j
...
*     Evaluate function at the endpoints
      ...
      write(*,*)  (fl .GT. 0.0d00 .AND. fh .GT. 0.0d00)
      write(*,*)   (fl .LT. 0.0d00 .AND. fh .LT. 0.0d00)
      write(*,*)   (fl * fh)
      
      IF ( (fl * fh) .GT. 0.0d00 ) THEN
         tryHarder = .TRUE.
         boundsOK = .FALSE.
 ...
 19   IF ( tryHarder ) THEN
     ...
     &       (fl .LT. 0.0d00 .AND. fh .LT. 0.0d00) ) THEN
        ...
        ELSE
           ...
           boundsOK = .TRUE.
        ENDIF
        IF ( (DABS(x1) .GT. 1.0d25) .OR. 
     &       (DABS(x2) .GT. 1.0d25 ) ) THEN
           boundsOK = .FALSE.
           tryHarder = .FALSE.
        ENDIF
