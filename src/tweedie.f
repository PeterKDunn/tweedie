...
*
***
*     IN:  double precision:  p, phi, y, mu, 
*          integer:           exact, verbose
*     OUT:  funvalue, exitstatus, relerr, its
***
*** NOTE:  WE SHOULD ALWAYS GET mu=1 FROM R

      double precision  p, phi, y, mu, 
     &                  funvalue, savemu,
...
*     SPECIAL CASES {
      if ( p .EQ. 1.0d00 ) then
*        We should never need to use this code: caught earlier
         funvalue = -10.d00
         return
      endif
...
*        Use exact zero acceleration algorithm
         ...
         if ( (p .GT. 1.0d00).AND.(p .LE. 2.0d00) ) then
            call smallp( p, phi, y, mu, aimrerr, 
     &                   result, maxit, ier, exitstatus,  
     &                   relerr, its, verbose )
...
*
*     OUT: FF, relerr, flag
*     IN:  psi, xvec, mmatrix, nmatrix, w, znum, wold, sumarea, verbose 
***
...
***
     ...
     &            fhi, dz1, dz2, z1, z2, sfzro, dk, dz
      ...
      external  dk, imgddcgf, imgdcgf, sfzro
...
*     The numbers are stored in  bp.txt  and wf.txt.
***
...
***
      ...
      bottom = exp( logb )
*     Appear to need this fix to get it to work...?      
...
*
*     IN: p, phi y, x1 (the lower bound), x2 (the upper bound), 
*         x0 (the best guess, between x1 and x2)
*     OUT: fun, dfun, ier
***
...
*****************************************************************
      ...
      subroutine smallp( p, phi, y, mu, aimrerr, 
     &                   result, maxit, ier, exitstatus, 
     &                   relerr, its, verbose )
...
*
*     CALLED BY: twpdf
*     IN:    double precision: p, phi, y, mu, aimrerr, 
*            integer:          verbose, ier, maxit
*     OUT:   result, its, exitstatus, relerr
***

      double precision  p, phi, y,  mu, aimrerr,
     &         relerr, result, zero1, zero2,
     &         flo, fhi, t0, area, 
     &         mmatrix(2, 200), nmatrix(2, 200),
     &         xvec(500), w, wold(3), area0, area1, 
     &         sumarea, pi,
     &         lower, upper, tstep, 
     &         zarea0, z1lo, z1hi, zdelta, resultp,
     &         f, f2, zerofn, zerodfn, sfzro
      integer  verbose, ier, maxit, 
     &         m, iteratn, flag, numzr,
     &         exitstatus, its, i, totalits, tier,
     &         sbuffer, goflag

      external  f, f2, zerofn, zerodfn, sfzro
...
*     FIND FIRST ZERO
      ...
      i = 0
      totalits = 1
*        Note: totalits = 1 (not 0) as the first region is done separately; 
...
*           Function values have same sign, so search wider for 
*           an interval containing the zero
...
*        This is linear interpolation between these places 
*        where we know the function crosses the axis:
