SUBROUTINE rtnewton(i, funcd, xstart, xacc, root)
  ...
  ! x_iter_old: the last safe value.
  ! x_new:      the proposed step value.
  ...
  DO j = 1, MAXITS
    ...
    ! Initialize step fraction and proposed new value
    factor = 1.0_C_DOUBLE
  ...
  IF (j .GE. MAXITS) THEN
    WRITE(*,*) "CONVERGENCE NOT OBTAINED IN RTNEWTON after", MAXITS, "iterations"
