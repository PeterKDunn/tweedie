SUBROUTINE findExactZeros(i, xL, xR, xStart, xZero) BIND(C, NAME='findExactZeros')
  ...
  INTERFACE
    ...
    SUBROUTINE funcd_signature(x, f, df) BIND(C)
      ! Re-added USE/IMPLICIT NONE for C_DOUBLE visibility in this interface block
      USE ISO_C_BINDING, ONLY: C_INT, C_DOUBLE
      IMPLICIT NONE 
    ...
    SUBROUTINE findImkM(t, f, df) BIND(C, NAME='findImkM')
      ! Re-added USE/IMPLICIT NONE for C_DOUBLE visibility in this interface block
      USE ISO_C_BINDING, ONLY: C_INT, C_DOUBLE
      IMPLICIT NONE
